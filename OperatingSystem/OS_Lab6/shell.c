...
// rndir命令用于修改目录名
int HandleRndir(char *oldName, char *newName) {
    int nameSz = strlen(newName);

    // Step 1: 检查新名字是否合法
    if (nameSz <= 0) {
        printf("Error: Missing operand for new directory name.\n");
        return -1;
    }
    for (int i = 0; i < nameSz; i++) {
        if (newName[i] == '/' || newName[i] == '-' || newName[i] == '.') {
            printf("Error: Illegal characters in directory name.\n");
            return -1;
        }
    }
    ...
    // Step 2: 查找旧目录
    FileDescriptor fatherFp;
    ReadFp(shellPath, &fatherFp); // 读取当前目录的描述符
    int bufSz = ((fatherFp.DIR_FileSize + SECTOR_SIZE - 1) / SECTOR_SIZE) * SECTOR_SIZE;
    char *buf = (char *)calloc(bufSz, sizeof(char));
    ReadData(&fatherFp, buf); // 读取当前目录的数据块
    ...
    for (int i = 0; i < bufSz; i += sizeof(FileDescriptor)) {
        ...
        if (tmpFp->DIR_Name[0] != 0) { // 非空目录项
            strcpy(tmpName, (char *)tmpFp->DIR_Name);
            if (strcmp(oldName, tmpName) == 0 && tmpFp->DIR_Attr == DIRECTORY_TYPE) {
                // 检查新名字是否已存在
                for (int j = 0; j < bufSz; j += sizeof(FileDescriptor)) {
                    FileDescriptor *conflictFp = (FileDescriptor *)(buf + j);
                    if (conflictFp->DIR_Name[0] != 0 && strcmp(newName, (char *)conflictFp->DIR_Name) == 0) {
                        printf("Error: Name conflict. '%s' already exists.\n", newName);
                        free(buf);
                        return -1;
                    }
                }
                ...
                // 重命名目录
                strncpy((char *)tmpFp->DIR_Name, newName, sizeof(tmpFp->DIR_Name) - 1);
                WriteData(buf, bufSz, &fatherFp);
                free(buf);
...
void ExecShell()
{
    ...
    while (1)
    {
        ...
        else if (strcmp(cmd[0], "rndir") == 0)
        {
            HandleRndir(cmd[1],cmd[2]);
        }
        else if (strcmp(cmd[0], "help") == 0)
        {
            printf("Commands Supported(Limited):\n"
                   "cat\tcd\tcopy\tload\tls\tmkdir\tpwd\tsave\trm\trmdir\ttouch\trndir\n"
