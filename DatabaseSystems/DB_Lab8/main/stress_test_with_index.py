# File: stress_test_with_index.py
import time
import threading
...
cursor = conn.cursor()
cursor.execute("TRUNCATE TABLE Menu")          # 清空数据
cursor.execute("CREATE INDEX idx_name ON Menu(name)")  # 创建非聚集索引
conn.commit()
cursor.close()
conn.close()

# 后续代码与模块2相同，仅修改日志标记为"QUERY_WITH_INDEX"
# 并发配置
...
def query_data():
    ...
    while True:
        ...
        log_operation("QUERY_WITH_INDEX", duration)
